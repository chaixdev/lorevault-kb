 ...
 */
public interface BookReductionClaimRepository extends Neo4jRepository<BookReductionClaim, String> {

    /**
     * Atomically try to create a {@link BookReductionClaim} for {@code bookId}/{@code lane}.
     *
     ...
     */
    @Query("""
            MERGE (c:BookReductionClaim {id: $claimId})
     ...
     */
    @Query("MATCH (c:BookReductionClaim {id: $claimId, acquiredToken: $acquiredToken}) DELETE c")
     ...
     */
    @Query("MATCH (c:BookReductionClaim) WHERE c.id IS NULL OR c.claimedAt IS NULL DELETE c")
