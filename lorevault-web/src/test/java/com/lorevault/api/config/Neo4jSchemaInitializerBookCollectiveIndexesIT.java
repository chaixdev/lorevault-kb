...
@Testcontainers
@Tag("integration")
class Neo4jSchemaInitializerBookCollectiveIndexesIT {
    ...
    @Test
    void ensuresBookCollectiveConstraintsAndIndex() {
        assertDoesNotThrow(() -> schemaInitializer.ensureMinimalSchema());

        assertThat(constraintExists("book_collective_id_unique")).isTrue();
        assertThat(constraintExists("book_collective_scope_unique")).isTrue();
        assertThat(indexExists("book_collective_book_name")).isTrue();
    }
