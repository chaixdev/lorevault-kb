...
@Testcontainers
@Tag("integration")
class Neo4jSchemaInitializerRelationClaimIndexesIT {
    ...
    @Test
    void ensuresRelationClaimConstraintsAndIndexes() {
        assertDoesNotThrow(() -> schemaInitializer.ensureMinimalSchema());

        assertThat(constraintExists("relation_claim_id_unique")).isTrue();
        assertThat(indexExists("relation_claim_chapter_reltype")).isTrue();
        assertThat(indexExists("relation_claim_book_reltype")).isTrue();
