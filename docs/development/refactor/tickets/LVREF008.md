...
public class ContentProcessingService {
    ...
    // Single service handling complete text → structured content pipeline
    public void processChapterContent(Chapter chapter) {
        // Complete processing workflow in one place
    }
}
```
...
**Rollback Plan**: Easy to remove if foundation proves insufficient for planned consolidation.