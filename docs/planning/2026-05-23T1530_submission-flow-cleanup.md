...
| P4-5 ✅ | **Audit `IngestionStatus` for orphaned values** — After Phase 1 removed all `updateJobStatus` calls. Found 6 orphaned values with zero production consumers: `QUEUED`, `PREPROCESSING_STARTED`, `EXTRACTING_ENTITIES`, `CHAPTER_EVENT_AGGREGATION`, `PERSISTING_DATA`, and `DETECTING_SCENES` (already @Deprecated). Marked all as @Deprecated with backward-compatibility notes. Not deleted — external API clients may reference these values. | Oracle review (G6) | 1 enum file | Resolved May 26. |
| P4-6 ✅ | **`SceneDetectionHandler` double chapter lookup** — Handler called `chapterRepo.findById(chapterId)` twice (lines 168 and 214), fetching the same entity for the same `chapterId`. Removed the second lookup at line 214, reusing the existing `chapter` variable. Saves one unnecessary DB round-trip. | Oracle review (G5) | 1 handler | Resolved May 26. |
