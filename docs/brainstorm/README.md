...
- [Event entity extraction staged solution design proposal — April 2026](entity-pipelines/event-entity-extraction-staged-solution-design-proposal-april-2026.md) — implementation-focused staged plan for building `EventMention`, conservative `ChapterEvent`, and thin `BookEvent` now while explicitly deferring `ResolvedEvent` and broader Event-DAG promotion
