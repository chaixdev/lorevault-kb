...
public class Neo4jContentPersistenceAdapterMultiJobIT extends IntegrationTestBase {
    ...
    void multipleJobs_statusOrdering() {
        ...
        job1.setCurrentStatusRecord(new StatusRecordNode(UUID.randomUUID(), job1.getId(), LocalDateTime.now(),
                IngestionStatus.QUEUED, "queued", IngestionStatus.QUEUED.getProgressPercentage(), null));
        ...
        job2.setCurrentStatusRecord(new StatusRecordNode(UUID.randomUUID(), job2.getId(), LocalDateTime.now(),
                IngestionStatus.QUEUED, "queued", IngestionStatus.QUEUED.getProgressPercentage(), null));
        job2 = port.createJob(job2);

        // Add status records (out of order creation) to ensure ordering retrieval works
        StatusRecordNode r2 = new StatusRecordNode();
        r2.setId(UUID.randomUUID());
        r2.setJobId(job2.getId());
        r2.setStatus(IngestionStatus.PREPROCESSING_STARTED);
        r2.setStepDescription("preprocessing");
        r2.setProgressPercent(10);
        port.addStatusRecord(job2.getId(), r2);
        StatusRecordNode r1 = new StatusRecordNode();
        r1.setId(UUID.randomUUID());
        r1.setJobId(job2.getId());
        r1.setStatus(IngestionStatus.QUEUED);
        r1.setStepDescription("queued");
        r1.setProgressPercent(0);
        port.addStatusRecord(job2.getId(), r1);
        ...
        // Most recent job should be the later one (job2) assuming createdAt
        // chronological
        assertThat(port.findMostRecentJobForChapter(chapter.getId()).map(IngestionJobNode::getId))
                .contains(job2.getId());
    }
}
